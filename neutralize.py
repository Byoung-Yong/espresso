...
import numpy as np
import pandas as pd
from reconstruct import load, simulate, NAMES
...
def main():
    ...
    for _,row in f.iterrows():
        t,p,q,w=load(str(row.csv_path)); x=np.array([row[n] for n in NAMES],float); qb,wb,*_=simulate(t,p,x,float(row.dose_g))
        for name,zero in NEUTRALIZATIONS.items():
            z=x.copy()
            for n in zero: z[NAMES.index(n)]=0.
            if name=="wetting_connectivity": z[NAMES.index("t_off")]=0.; z[NAMES.index("wetting_tau")]=.1
            q2,w2,*_=simulate(t,p,z,float(row.dose_g)); out.append({"csv_path":row.csv_path,"neutralization":name,"flow_r2_loss":r2(q,qb)-r2(q,q2),"mass_r2_loss":r2(w,wb)-r2(w,w2),"baseline_flow_r2":r2(q,qb),"baseline_mass_r2":r2(w,wb)})
